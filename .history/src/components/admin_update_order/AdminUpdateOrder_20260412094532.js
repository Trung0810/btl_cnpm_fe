...
const AdminUpdateOrder = () => {
  ...
  const total = useMemo(() => {
    ...
    return watchItems.reduce((sum, item) => {
      const shoes = shoesData.find((s) => s._id === item.shoesId);

      const price = shoes?.price || 0;
      const quantity = Number(item.quantity) || 0;

      return sum + price * quantity;
    }, 0);
  }, [watchItems, shoesData]);
              ...
              {fields.map((field, index) => (
                <tr key={field.id}>
                  <td style={{ display: "none" }}>
                    <input {...register(`items.${index}.shoesId`)} />
                  </td>
                  <td>
                    <select
                      name="name"
