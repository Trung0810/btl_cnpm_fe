...
const AdminUpdateOrder = () => {
  ...
  const [shoesNameList, setShoesNameList] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const shoesData = shoesResponse.data.data;
        const shoesNameList = shoesData.map((item) => item.name);
        reset({
          ...
          <table>
            ...
            <tbody>
              {fields.map((field, index) => (
                <tr key={field.id}>
                  <td>
                    <select {...register(`items.${index}.name`)}>{}</select>
