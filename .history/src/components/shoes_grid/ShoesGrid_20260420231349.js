    ...
    const fetchData = async () => {
      try {
        ...
        const [shoesResponse, quantityResponse] = await Promise.all([
          axios.get(
            `http://localhost:8888/v1/api/shoes?keyword=${filter.keyword}&brand=${filter.brand}&type=${type}&category=${filter.category}&price=${filter.price}&rate=${filter.rate}&sort=${filter.sort}&page=${currentPage}&limit=${limit}`,
          ),
          axios.get(
