...
const ShoesGrid = () => {
  ...
  const { customer, path, setPath, currentPage, setCurrentPage, filter } =
    ...
    const fetchData = async () => {
      try {
        ...
        const [shoesResponse, quantityResponse] = await Promise.all([
          axios.get(
            `http://localhost:8888/v1/api/shoes?name=${filter.name}&brand=${filter.brand}&type=${type}&category=${filter.category}&price=${filter.price}&rate=${filter.rate}${filter.sort ? `&sort=${filter.sort}` : ""}&page=${currentPage}&limit=${limit}`,
          ),
          axios.get(
            `http://localhost:8888/v1/api/quantities?name=${filter.name}brand=${filter.brand}&type=${type}&category=${filter.category}`,
          ),
        ]);
