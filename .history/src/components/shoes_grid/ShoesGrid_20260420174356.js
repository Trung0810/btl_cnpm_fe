...
const ShoesGrid = () => {
  ...
  const { page, setPage, filter } = useContext(InforContext);
    ...
    const fetchData = async () => {
      try {
        ...
        const [shoesResponse, quantityResponse] = await Promise.all([
          axios.get(
            `http://localhost:8888/v1/api/shoes?brand=${filter.brand}&type=${type}&category=${filter.category}&page=${page}&limit=${limit}`,
          ),
          axios.get(`http://localhost:8888/v1/api/quantities?type=${type}`),
        ]);
      ...
      <div className="shoes-bar">
        {arrayPage.map((page, index) => (
          <div key={index} className="shoes-bar-page">
            <input type="radio" name="page" id={page} />
            <label htmlFor={page} onClick={() => setPage(page)}>
              {page}
            </label>
          </div>
        ))}
