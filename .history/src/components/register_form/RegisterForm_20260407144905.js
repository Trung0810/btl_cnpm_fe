...
const RegisterForm = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const birthday = dayjs(data.birthday).format("DD/MM/YYYY");
      const res = await axios.post("http://localhost:8888/v1/api/customers", {
        ...data,
        birthday,
      });
      ...
      console.log("🚀 ~ onSubmit ~ e:", e.response.data.error);
      setStatus("failed");
      setTitle("Create account failed!");
      setDesc(e.response.data.error);
      setShow(true);
    }
  };
  ...
  const handleShow = () => {
    setShow(false);
    if (status === "success") nevigate("/login");
  };
